...
namespace Rhetorica
{
  public sealed class Repository
  {
    ...
    private static string _nlpTextsFolder = string.Empty;
    ...
    private Repository()
    {
      ...
      _rootDrive = (ConfigurationManager.AppSettings["RootDrive"]).Replace(@"\", Dsc);
      _nlpFolder = (ConfigurationManager.AppSettings["NlpFolder"]).Replace(@"\", Dsc);

      _openNlpModelsFolder = (ConfigurationManager.AppSettings["OpenNlpModelsFolder"]).Replace(@"\", Dsc);
      _openNlpModelsPath = RootDrive + _nlpFolder + _openNlpModelsFolder;

      _wordNetFolder = (ConfigurationManager.AppSettings["WordNetFolder"]).Replace(@"\", Dsc);
      _wordNetPath = RootDrive + _nlpFolder + _wordNetFolder;

      _grammarFolder = (ConfigurationManager.AppSettings["StanfordParserGrammarFolder"]).Replace(@"\", Dsc);
      _grammarPath = RootDrive + _nlpFolder + _grammarFolder;

      _nlpTextsFolder = (ConfigurationManager.AppSettings["NlpTextsFolder"]).Replace(@"\", Dsc);
      _nlpTextsPath = RootDrive + _nlpFolder + _nlpTextsFolder;
      ...
      _localTextPath = Path.Combine(localTextDirectoryParts) + Dsc; // For development use
